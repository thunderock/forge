...
export function NewTaskDialog(props: NewTaskDialogProps) {
  ...
  // Per-agent-id model choice for this task (a map so Phase 6 fan-out panes reuse it).
  ...
  async function handleSubmit(e: Event) {
    ...
    // Carry the model choice on a COPY of the AgentDef — never mutate the shared
    // availableAgents entry. buildTaskAgentArgs reads these at launch (MDL-02).
    const modelSel = modelSelections()[agent.id] ?? {};
    const agentDefWithModel: AgentDef = {
      ...agent,
      ...(modelSel.model?.trim() ? { model: modelSel.model.trim() } : {}),
      ...(modelSel.reasoningEffort?.trim()
        ? { reasoningEffort: modelSel.reasoningEffort.trim() }
        : {}),
    };
    ...
    try {
      ...
      // Remember this agent's model choice globally for next time (MDL-05).
      setLastModelSelection(agent.id, modelSel);
