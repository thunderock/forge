...
//  - flush: the output-handler-driven drain. Gates on the ~50ms `stability`
//    window (a marker must persist before a queued item flushes) because this
//    path observes per-chunk output where a marker can flicker.
...
export function decideBroadcastDelivery(p: BroadcastDecisionParams): BroadcastDecision {
  ...
  const base = p.ready && !p.questionActive && !p.writeLocked && !suppressed;
  ...
  // flush phase — stability is the gate (idle is passed but not required so one
  // fn contract serves both phases).
  if (base && stable) return { action: 'flush' };
